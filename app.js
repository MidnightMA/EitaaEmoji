...
function renderAppleEmojis(text) {    
    ...
    if (segmenter) {
        for (let {segment} of segmenter.segment(text)) {
            ...
            const imgUrl = `https://cdn.jsdelivr.net/npm/emoji-datasource-apple@15.0.1/img/apple/64/${cleanHex}.png`;
            ...
            html += `
                <img src="${imgUrl}" 
                     ...
                     alt="${segment}" 
                     loading="eager" 
                     onerror="this.style.display='none'; this.parentElement.innerHTML += this.alt;">
            `;
        }
    } else {
        // fallback regex
        html = text.replace(/([\u{1f300}-\u{1f9ff}\u{2600}-\u{26ff}\u{2700}-\u{27bf}])/gu, match => {
            // مشابه بالا + onerror
        });
    }
